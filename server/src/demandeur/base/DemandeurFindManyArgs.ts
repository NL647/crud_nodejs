/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { DemandeurWhereInput } from "./DemandeurWhereInput";
import { Type } from "class-transformer";
import { DemandeurOrderByInput } from "./DemandeurOrderByInput";

@ArgsType()
class DemandeurFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => DemandeurWhereInput,
  })
  @Field(() => DemandeurWhereInput, { nullable: true })
  @Type(() => DemandeurWhereInput)
  where?: DemandeurWhereInput;

  @ApiProperty({
    required: false,
    type: [DemandeurOrderByInput],
  })
  @Field(() => [DemandeurOrderByInput], { nullable: true })
  @Type(() => DemandeurOrderByInput)
  orderBy?: Array<DemandeurOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { DemandeurFindManyArgs };
